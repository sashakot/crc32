...
#else
#pragma message "NO __ARM_FEATURE_CRC32"
#endif

static const uint32_t ieee_table[256] = {
				  ...
				  const size_t length)
{
	...
	for (size_t i = 0; i < count; i++) {
		crc = (crc << 8) ^ ieee_table[(crc >> 24) ^ (*data++)];
	}
	...
	for (size_t i = 0; i < count; i++) {
		const uint64_t d64 = *data64++;

#if __BYTE_ORDER == __LITTLE_ENDIAN
		crc = (crc << 8) ^ ieee_table[(crc >> 24) ^ (d64 & 0xff)];
		crc = (crc << 8) ^ ieee_table[(crc >> 24) ^ ((d64 >> 8) & 0xff)];
		crc = (crc << 8) ^ ieee_table[(crc >> 24) ^ ((d64 >> 16) & 0xff)];
		crc = (crc << 8) ^ ieee_table[(crc >> 24) ^ ((d64 >> 24) & 0xff)];
		crc = (crc << 8) ^ ieee_table[(crc >> 24) ^ ((d64 >> 32) & 0xff)];
		crc = (crc << 8) ^ ieee_table[(crc >> 24) ^ ((d64 >> 40) & 0xff)];
		crc = (crc << 8) ^ ieee_table[(crc >> 24) ^ ((d64 >> 48) & 0xff)];
		crc = (crc << 8) ^ ieee_table[(crc >> 24) ^ ((d64 >> 56) & 0xff)];
#else
		crc = (crc << 8) ^ ieee_table[(crc >> 24) ^ ((d64 >> 56) & 0xff)];
		crc = (crc << 8) ^ ieee_table[(crc >> 24) ^ ((d64 >> 48) & 0xff)];
		crc = (crc << 8) ^ ieee_table[(crc >> 24) ^ ((d64 >> 40) & 0xff)];
